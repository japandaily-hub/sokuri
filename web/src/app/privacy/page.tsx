...
const COLLECTED: { type: string; how: string; detail: string }[] = [
  { type: "アカウント情報", how: "会員登録時", detail: "メールアドレス、パスワード、氏名（任意）、LINE連携時はLINEユーザーID" },
  { type: "プロフィール情報", how: "マイページでの任意入力", detail: "氏名（姓・名・フリガナ）、電話番号、お住まいのエリア" },
  { type: "本人確認情報", how: "マイページからの任意提出", detail: "運転免許証・マイナンバーカード等の本人確認書類の画像、生年月日、職業（なりすまし・不正出品の防止、および当社からの本人確認のために利用します。登録業者へ提供することはありません）" },
  { type: "振込口座情報（ユーザー）", how: "マイページでの任意入力", detail: "銀行名・支店名・預金種別・口座番号・口座名義（暗号化して保存し、ご本人のみが内容を確認できます。登録業者へ自動で開示することはありません）" },
  ...
  { type: "お問い合わせ情報", how: "お問い合わせフォーム送信時", detail: "お名前、メールアドレス、お問い合わせ種別、お問い合わせ内容（回答のためにのみ利用します）" },
