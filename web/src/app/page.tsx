...
import { LpChrome } from "./lp/_components/LpChrome";
import { LpSlider, type LpSlide } from "./lp/_components/LpSlider";
import { LpRibbon } from "./lp/_components/LpRibbon";
import { LpIntro } from "./lp/_components/LpIntro";
import { D, FloatDeco, type DecoIll } from "./lp/_components/FloatDeco";
import "./lp/lp.css";

/** 2026-09-18 本採用: 参照LP（はばたき農場）移植のデザインを正式なトップページに昇格。
 *  旧トップは /top-classic へ退避（noindex）。検索インデックス対象（robots 指定なし=既定で index）。 */
export const metadata: Metadata = {
  title: "カタヅケ｜家まるごと、まとめて片付け買取",
  description:
    "家じゅうの不用品を、1点ずつ撮って、あとは待つだけ。登録業者が買取総額で競い合い、連絡が来るのはあなたが選んだ1社だけ。値がつかない物もまとめて引き取り。営業電話に追われない、家まるごとの片付け買取マッチング。",
  alternates: { canonical: "/" },
};

/* ============================================================
   KV スライダー（参照 `.home-kv__slider` 4枚 → カタヅケは 4枚）
   トップページの HeroCarousel と同じ思想（誰が使うサービスかを一人の像に固定しない）で 4 つのペルソナを回す。
   2026-09-18 ユーザー指摘: 「困っている瞬間」の絵は不満そうでイメージが悪い → 4 枚とも
   「片付いて、気持ちがいい」状態（片付いた明るい部屋でくつろぐ）の絵に差し替え。見出し（片付けたい。でも、
   動けないあなたへ。）は共感、写真は到達点、という役割分担にする。
   ============================================================ */
const KV_SLIDES: LpSlide[] = [
  { id: "kv-tidy-woman", src: "/img/lp/kv-tidy-woman.webp", alt: "片付いた明るいリビングのソファで、お茶を手にくつろぐ30代半ばの女性（イメージ）" },
  { id: "kv-tidy-man", src: "/img/lp/kv-tidy-man.webp", alt: "片付いた明るいリビングのソファで、ゆったり座る30代の男性（イメージ）" },
  { id: "kv-tidy-family", src: "/img/lp/kv-tidy-family.webp", alt: "片付いた広いリビングのラグで、子どもと笑い合う30代の夫婦（イメージ）" },
  { id: "kv-tidy-couple-60s", src: "/img/lp/kv-tidy-couple-60s.webp", alt: "片付いた明るいリビングのソファで、お茶を飲みながら微笑み合う60代の夫婦（イメージ）" },
];
...
const ABOUT_ITEMS: AboutItem[] = [
  {
    ...
    body: (
      <>
        ...
        の片付け向け。1点ずつ売るのではなく、たまった不用品をまとめて査定へ。まとめて出すほど買取総額が伸びやすく、値がつかない物も一緒に手放せます。
      </>
    ),
    img: {
  ...
  },
  {
    ...
    body: <>あなたが出すのは写真だけ。あとは登録業者どうしが、出品された品物に買取総額で入札し合います。</>,
    img: {
...
const POINT_BLOCKS: PointBlock[] = [
  {
    ...
    minors: [
      ...
      },
      {
        ...
        p: (
          <>
            業者は1点ごとではなく<strong className="lp-mk">出品した品物すべてに対する買取総額で入札</strong>
            します。だから単体では値がつきにくい物も、他の品物と一緒に引き取ってもらえます。「これは売れないかも」も、一緒に手放せます。
          </>
        ),
      },
      {
    ...
    cells: [
      {
        n: "02",
        h: "査定が届く",
        p: <>登録業者が、あなたの出品した品物を査定して入札。あなたは待つだけで、査定額が集まります。</>,
  ...
  },
  {
    ...
    minors: [
      { h: "届くのは出品内容だけ", p: <>地域は都道府県・市区町村まで。番地などの詳細住所は含まれません。</> },
      {
        h: "氏名・電話は渡りません",
        p: <>お名前や電話番号が業者に渡ることはありません。詳細住所と連絡用のメールアドレスは、交渉が成立した1社にのみ開示されます。</>,
      },
      { h: "詳細住所とメールは交渉成立後", p: <>交渉成立後に連絡先が開示され、業者と引き取り日時を決めます。</> },
    ],
    cells: [
      {
        n: "02",
        h: "登録業者が買取総額で入札",
        p: <>複数の業者が、出品した品物すべてに対して買取総額を提示します。提示された総額は、すべて一覧で見比べられます。</>,
        img: {
      ...
      },
      {
        n: "03",
        h: "連絡が来るのは選んだ1社だけ",
        p: <>選ぶまで、業者はあなたに連絡できません。選ばなかった業者には自動でお断りが入ります。複数の業者から一斉に営業電話がかかってくることはありません。</>,
        img: {
  ...
  },
  {
    ...
    main: {
      ...
      p: (
        <>
          査定に参加するのは、登録された買取業者だけです。古物商許可の確認と審査を通過した業者から、順に参加します。
        </>
      ),
      img: {
        src: "/img/real/trust-illus-1.webp",
        alt: "登録業者の古物商許可を確認するイメージ",
        w: 1024,
        h: 768,
        contain: true,
      },
    ...
    minors: [
      { h: "古物商許可を確認", p: <>古物商許可を、登録時・取引前に確認します。</> },
      { h: "東京・千葉・埼玉・神奈川", p: <>対応エリアは順次拡大中です。</> },
      { h: "審査を通過した業者から順に参加", p: <>登録時の審査を通過した業者だけが、査定に参加します。</> },
    ],
    cells: [
      {
        n: "02",
      ...
      },
      {
        ...
        p: (
          <>
            訪問による買取には特定商取引法（訪問購入）の規定が適用される場合があります。一部の物品は法令により対象外とされています。あなたの品物が対象かどうかは、訪問した業者が交付する書面に記載されます。ご不明な場合は消費者ホットラインにご相談ください。
          </>
        ),
        img: {
...
const DAILY_STEPS: DailyStep[] = [
  ...
  },
  {
    ...
    tag: "業者が競う",
    trail: true,
    p: "複数の業者が、出品した品物すべてに対して買取総額を提示します。",
  },
  { n: "05", img: "/img/lp/compare-offers.webp", w: 1024, h: 1536, pos: "50% 30%", tag: "見比べて選ぶ", trail: true, p: "提示された総額は、すべて一覧で見比べられます。" },
  {
    ...
    h: 1536,
    pos: "50% 45%",
    p: "見比べて、納得できる1社を選択。選ばなかった業者には自動でお断りが入ります。",
  },
  { n: "07", img: "/img/lp/schedule-visit.webp", w: 1024, h: 1024, trail: true, p: "交渉成立後に連絡先が開示され、業者と引き取り日時を決めます。" },
  {
    n: "08",
  ...
  return (
    ...
    <div className="lp-page">
      ...
      <main id="main">
        ...
        <section id="founder" className="lp-message" aria-label="運営事務局からのメッセージ">
        ...
        <div id="point" className="lp-point">
          {POINT_BLOCKS.map((b, bi) => (
            <section id={b.id} className={`lp-point__block lp-point__block--${b.tone}`} key={b.id} aria-labelledby={`lp-point-${b.id}`}>
          ...
          <div className="lp-container">
            <Reveal className="lp-fee__panel" variant="up">
              ...
              <div className="lp-fee__zero">
                ...
                <span className="lp-fee__note">出品・査定・お断り・引き取りの、どの段階でも無料です。</span>
          ...
          <div className="lp-cases__intro">
            ...
            <div className="lp-container">
              ...
              <Reveal className="lp-cases__introbody" variant="up">
                <h2>こんなふうに使えます</h2>
                <p className="lp-en">model cases</p>
              </Reveal>
            </div>
          </div>
        ...
        <section id="biz" className="lp-biz">
          ...
          <div className="lp-container lp-biz__inner">
            ...
            <p className="lp-biz__req">※ ご参加には古物商許可が必要です。</p>
