...
- **手動実行**: `POST /admin/jobs/reminders`（管理者トークン、または後述の `X-Ops-Token`）。応答は `{"overdue": n, "no_bid": n, "bids_pending": n, "undelivered": m, "unreachable": k}`。n 系は今回リマインド対象として確保した件数（宛先不達・通知失敗の分も含む）、`undelivered`（m）は実際に届かなかった件数、`unreachable`（k）は試行できる宛先が無かった件数（下記参照）。動作確認や、キルスイッチで自動実行を止めている間に手で1周回したいときに使う。
...
- **再送したい / 送らないようにしたい**: 送信済みの印は `transactions.overdue_reminded_at` / `cases.no_bid_reminded_at` / `cases.bids_pending_reminded_at`。再送は DB で該当行を NULL に戻す（管理画面の操作は用意していない）。通知の送信自体（LINE・メールとも）に失敗した場合は再送されない。1周あたりの配送結果は `notify_dispatch.dispatch_*` の戻り値（delivered/failed/skipped）で種別ごとに集計し、`run_reminders` が3種別（overdue/no_bid/bids_pending）を合算して**1周につき最大1本**の運営 LINE へ warning アラート（`key=reminder_dispatch_failed`）を飛ばす。本文には不達件数の総数・種別内訳・失敗した**対象の識別子**（transaction_id/case_id と依頼者/業者の別。メールアドレスや LINE ユーザーIDなどの宛先そのものは含めない）の先頭5件・「マーカー確定済みのため再送されない」旨を含む。次の周で不達が0件に戻ると復旧通知（resolve）が対で飛ぶ（確保0件の空周では復旧通知は出さない）。なお、LINE 未連携かつ仮メール等で**そもそも試行できる宛先が無かった**ものは `unreachable` に計上されるだけでアラート対象外（失敗ではないため）。ただし対象が10件以上あるのに**全件** unreachable（0件送信）になった場合は、LINE連携やメール判定のデータ不整合を疑い、別キー（`key=reminder_all_unreachable`）で系統障害として warning を飛ばす（1〜2件程度の unreachable は平常運転でも起こるため対象外）。
