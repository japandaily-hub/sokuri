...
8. [ ] **弁護士・行政書士に確認（開業前・優先度上昇）** — **2026-09-07 の方針転換（業者は成約まで入札額を何度でも引き上げ可＋他社の入札額を匿名開示）により、仕組みは実質「公開の競り上げ」に近づいた。r12 で低減した該当性が再び上がるため、公開前に確認必須。** (a) 入札方式が古物営業法「古物競りあっせん業」に当たるか（当たれば営業開始2週間以内に公安委員会へ届出。**前提が r12 で変わった: 他社の入札額は業者に非開示。業者に見えるのは入札件数と自社の入札のみ**で、業者間で額を競り上げる仕組みではない＝「競り」に当たらないという整理が可能か、が論点） (b) 「依頼者の求めで業者が訪問する」形態が特商法58条の17第2項1号の「請求者への訪問購入」に当たりクーリング・オフ非適用となるか（現行文言は非断定にしてある）。
...
- [x] **2026-09-25 レビューで見つかった残り（Low）→ 同日対応** — ①Gemini へ送る画像は全て `vision.analyze_image` で除去を通す（`/api/v1/analyze` を含む単一の関門。MIME は実バイトから判定、除去できない画像は 422。入力は復号後 10MB まで・`base_image` に長さ上限。利用者に返すのは入力起因の固定文言だけで、他の ValueError は固定文言＋ログ）②`web/package.json` に `engines.node: "24.x"`（Vercel の現行設定 24.x・CI・ローカルと一致させて固定。本番ビルドの Node は変わらない）③`admin.py` の既存 Lint（未使用の `text`・ループ変数 `case` を `case_row` へ）④`scripts/pg_concurrency_check.py` に S7（最後の管理者2人の同時退会・相互降格・退会と降格の同時実行で管理者が 0 人にならない）を追加し CI の実 PostgreSQL で検証（接続先がローカル以外なら拒否・運営アカウントの自己修復つき）。ロック句が消えたら落ちる単体テスト `tests/test_last_admin_lock_sql.py`（PostgreSQL 方言でコンパイルして FOR NO KEY UPDATE と id 順を確認）。
