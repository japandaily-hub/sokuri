...
- [ ] **r10 の意図的未対応** — Brevo が 201 受理後に破棄する経路の検知（日次で送信統計 API を照会）／`/admin/contacts` の検索／`contact_messages` の保持期間の明文化／訪問日超過・入札ゼロのリマインド（未実装機能）。（旧項目「審査中の自己登録業者が公開案件の写真を閲覧できる（TODO 01-4）」は決定: 承認まで閲覧不可。backend が `/cases`・`/cases/{id}`・`/cases/{id}/bids`・案件写真を 403 `approval_required` で拒否し、web は `ApprovalPendingNotice` に差し替え済み・2026-09-06）
