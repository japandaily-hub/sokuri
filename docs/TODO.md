...
4. [x] **招待コード登録の業者を、許可証未提出でも active にしてよいか決める** — **決定（2026-09-25）: 許可証必須。** 招待コード経由でも signup 直後は pending、許可証画像の提出＋運営承認で active（0d6005d・本番反映済み）。**残: 旧仕様で許可証なしのまま active の業者が本番に 1 社**（0043 の監査ログで確認・値は未変更）。提出を依頼するか pending に戻すかはユーザー判断。
5. [ ] **（保留・2026-09-25 ユーザー判断）事業計画書の空欄を埋める** — 代表者略歴・自己資金の形成過程。成約単価 45,000 円・成立率 60% はβ実績で更新。
...
7. [ ] **r3 で置いた文言の仮定を承認・差し戻し** — **2026-09-25 決定: ②③は承認。①は差し戻し＝決済方針が決まるまで代金の流れを断定しない（「お支払い方法は成約後にご案内します」へ、特商法表記を含め全箇所修正・本番反映済み 0d6005d/96371e9）。④所在地はユーザーが正しい住所を後で伝える（未反映）。** 旧記載: ①買取代金は業者と依頼者の当事者間精算（カタヅケは送金しない）、口座登録は「振込希望時に業者へ伝える口座」 ②入札期間の上限なし（依頼者が業者を選んだ時点で終了） ③手数料8%（税別・消費税を別途加算、2026-09-06決定）は残し「β期間は請求しない」注記 ④事業者所在地は神奈川県横浜市。差し戻す場合は該当ページ（legal/faq/terms/privacy/mypage/bank-account）を修正。
8. [ ] **弁護士・行政書士に確認（開業前・優先度上昇）** — **2026-09-25: 相談資料を作成済み `docs/legal_consultation_katazuke_20260925.md`（古物競りあっせん業・資金決済法・特商法 訪問購入・手数料開始前の要件、質問25問）。決済方針（当事者間かカタヅケ経由か）は相談後に決める。【記入】欄（代表者名・連絡先等）の記入と相談の実施はユーザー。** なお下の (a) の「他社の入札額は非開示」は 09-07 以降の実装（匿名開示）と異なる。資料は実装に合わせた。旧記載: **2026-09-07 の方針転換（業者は成約まで入札額を何度でも引き上げ可＋他社の入札額を匿名開示）により、仕組みは実質「公開の競り上げ」に近づいた。r12 で低減した該当性が再び上がるため、公開前に確認必須。** (a) 入札方式が古物営業法「古物競りあっせん業」に当たるか（当たれば営業開始2週間以内に公安委員会へ届出。**前提が r12 で変わった: 他社の入札額は業者に非開示。業者に見えるのは入札件数と自社の入札のみ**で、業者間で額を競り上げる仕組みではない＝「競り」に当たらないという整理が可能か、が論点） (b) 「依頼者の求めで業者が訪問する」形態が特商法58条の17第2項1号の「請求者への訪問購入」に当たりクーリング・オフ非適用となるか（現行文言は非断定にしてある）。
