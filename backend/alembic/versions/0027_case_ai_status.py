...
- ``ai_status``: "pending" / "done" / "failed"。既存行は解析完了済みのため "done" で埋める
  （server_default は新規行向けの "pending"）。
...
書き換え無し）＋索引2本。現在の行数規模では ACCESS EXCLUSIVE は一瞬で済む。
"""
...
def upgrade() -> None:
    ...
    # 既存案件は作成時に同期解析済み（結果が ai_summary に入っている）ため "done" にする。
    # 新規行のみ server_default の "pending" から始まる。
    op.execute("UPDATE cases SET ai_status = 'done'")
