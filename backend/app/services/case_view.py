...
``bid_count`` / ``my_bid`` は入札関連情報（``case.bids``）を参照する cases.py 側
のみで追加設定する。transactions.py は成約が既に確定した文脈のため従来通り
これらを設定しない（デフォルト値のまま）。

r12 決定2 により、他社の入札額（旧 ``top_bid_amount``）は業者向け応答から削除した
（古物競りあっせん業の該当性を下げるため、競り＝他社額・順位の外形を出さない）。
"""
