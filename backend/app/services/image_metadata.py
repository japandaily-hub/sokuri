...
# （と MPF）は TIFF ヘッダを、XMP は Adobe の名前空間を必ず含む。いずれも 8 バイト以上で、
# 画素データや ICC プロファイルと偶然一致する確率は無視できる。
...
def _strip_jpeg(data: bytes) -> bytes:
    ...
    - 壊れたファイルへの備え: 正しいファイルのスキャンデータにメタデータは入らないが、マーカー
      が欠けると後続の Exif / XMP がスキャンデータとして吸収され得る。スキャンデータと残す
      セグメントの中身に Exif（TIFF ヘッダ）・XMP の目印があれば例外、SOF の重複・SOF より前の
      SOS（libjpeg も致命エラー）も例外、ICC・サムネイル付き JFIF の直後で再同期が起きたら
      中身が削られて後続を飲み込んだ疑いがあるため ICC は全部・JFIF はサムネイルを落とす。
    """
    ...
    # 直後で再同期が起きたら長さと中身が食い違っている（中身が削られて後続のセグメントの
    # 先頭を飲み込んだ等）とみなし、ICC は全部落とし、JFIF はサムネイルを落とす。
    icc_just_kept = False
    ...
    for _ in range(_MAX_JPEG_SEGMENTS):
        ...
        if marker == _MARKER_SOS:
            ...
            if _contains_metadata_signature(data, segment_end, scan_end):
                raise ImageMetadataError(
        ...
        if marker in _IMAGE_STRUCTURE_MARKERS:
            ...
            if data.startswith(_JFIF_IDENTIFIER, payload_start, segment_end):
                ...
                if kept_size > _JFIF_FIXED_SIZE:
                    jfif_thumbnail_just_kept = (len(output), marker_start, segment_end)
...
def _settle_icc_chunks(
    ...
    疑いがある場合（``tainted``: いずれかのチャンクの直後で再同期が起きた／残す範囲に
    Exif・XMP の目印がある）は全部落とす。Chromium（libjpeg-turbo の jpeg_read_icc_profile と
    skcms）も前者の ICC は使わないため表示は変わらず、後者も色が sRGB 扱いになるだけで写真
    自体は配信できる。
    """
    if not chunks:
        return
    ...
    if all(chunk.count == count for chunk in chunks) and sequences == list(range(1, count + 1)):
        ...
        if first.segment_end - first.data_start >= 4:
            ...
            if profile_size >= _ICC_HEADER_SIZE:
                if stored_size == profile_size and not any(
                    _contains_metadata_signature(data, chunk.data_start, chunk.segment_end)
                    for chunk in chunks
                ):
                    return
                ...
                if (
                    count == 1
                    and stored_size > profile_size
                    and not _contains_metadata_signature(data, first.data_start, profile_end)
                ):
...
def _read_exif_orientation(payload: bytes) -> tuple[int, str] | None:
    ...
    if not payload.startswith(_EXIF_IDENTIFIER):
        return None
    tiff = payload[_EXIF_HEADER_SIZE:]
    byte_order = tiff[:2]
    if byte_order == b"II":
        endian = "<"
...
def _build_orientation_only_exif(orientation: int, endian: str) -> bytes:
    ...
    構成: ``"Exif\\0\\0"`` + TIFF ヘッダ（バイト順・42・IFD0 オフセット 8）+ IFD0
    （エントリ数 1・Orientation / SHORT / 個数 1 / 値・次 IFD オフセット 0）。GPS IFD・
    Exif IFD・サムネイル（IFD1）へのポインタは持たない。バイト順は元の Exif に合わせる。
    """
...
# ものに限る。eXIf・tEXt・zTXt・iTXt（XMP を含む）・tIME のほか、caBX（C2PA）・旧仕様の
# exIf・アプリ独自チャンク等、ここに無いものはすべて落とす。
...
def _strip_png(data: bytes) -> bytes:
    ...
    for _ in range(_MAX_PNG_CHUNKS):
        ...
        if is_critical or chunk_type in _PNG_KEPT_ANCILLARY_TYPES:
...
_VP8X_FLAG_XMP = 0x04
# 残すチャンク（WebP コンテナ仕様で画像の表示に関わるもの）。EXIF・"XMP "・未知の
# チャンク（C2PA 等）は落とす。ANMF（アニメーションのフレーム）は中身を解釈せずに残す。
